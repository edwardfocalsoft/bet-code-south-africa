...
const SellerDashboard: React.FC = () => {
  ...
  const chartData = performanceData.map(item => ({
    period: item.name,
    sales: item.sales,
    revenue: item.sales * 100,
    // Approximate revenue for chart
    avgRating: averageRating || 0
  }));
  if (isLoading) {
    return <Layout requireAuth={true} allowedRoles={["seller", "admin"]}>
  ...
  return <Layout requireAuth={true} allowedRoles={["seller", "admin"]}>
      <div className="container mx-auto py-8">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
          <h1 className="text-3xl font-bold">Tipster Dashboard</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <StatCard title="Total Sales" value={totalSales} icon={TrendingUp} trend={{
          value: 12,
          isPositive: true
        }} />
          <StatCard title="Active Tickets" value={activeTickets} icon={Ticket} trend={{
          value: 5,
          isPositive: true
        }} />
          <StatCard title="Total Revenue" value={`R${totalRevenue.toLocaleString()}`} icon={DollarSign} trend={{
          value: 8,
          isPositive: true
        }} />
          <StatCard title="Average Rating" value={averageRating?.toFixed(1) || "0.0"} icon={Star} trend={{
          value: 2,
          isPositive: true
        }} />
        </div>
        ...
        <div className="mb-8">
          <SinglePerformanceChart data={chartData} isLoading={isLoading} />
        </div>
