...
const HeroSection: React.FC = () => {
    ...
    const fetchStats = async () => {
      ...
      try {
        ...
        const {
          data: profilesData,
          error: profilesError
        } = await supabase.from("profiles").select("id, role").or('role.eq.buyer,role.eq.seller');
        ...
        const {
          data: winData,
          error: winError
        } = await supabase.from("purchases").select("is_winner");
        ...
        const {
          data: payoutData,
          error: payoutError
        } = await supabase.from("purchases").select("price").eq("payment_status", "completed"); // Only count completed purchases
  ...
  return <section className="bg-gradient-to-br from-betting-black via-betting-dark-gray to-betting-black pt-16 pb-24 px-4 relative overflow-hidden">
      ...
      <div className="container mx-auto text-center relative z-10">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center mb-12 animate-fade-in">
          <Link to="/tickets">
            <Button className="bg-betting-green hover:bg-betting-green-dark text-white px-8 py-6 text-lg w-12/12 ">
              Browse Tickets
            </Button>
          </Link>
