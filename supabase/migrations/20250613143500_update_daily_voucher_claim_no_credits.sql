...
-- Update the claim_daily_voucher function to not add credits
...
BEGIN
  ...
  -- Create the claim (no credit addition)
  INSERT INTO public.voucher_claims (voucher_id, user_id)
  VALUES (p_voucher_id, p_user_id);
  
  -- Create wallet transaction record for tracking (but no actual credit balance change)
  INSERT INTO public.wallet_transactions (
    ...
    p_user_id,
    0, -- Set to 0 since we're not adding credits
    'voucher',
    'Daily voucher claim: ' || v_voucher.code || ' (External voucher code)',
    p_voucher_id
  );
